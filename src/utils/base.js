...
function direction(type, value) { 
  return css`
   ${type} : ${value};
  `
}

function marginPosition(type, value) {
  return css`
  margin-${type} : ${value};
 `
}
...
function btn(background, color) {
  ...
  transition: all 0.4s ease-in-out 0s;
  &:focus{
    background: 0 0px 0px 0 ${background};
    outline: 0;
  }
  `
}
